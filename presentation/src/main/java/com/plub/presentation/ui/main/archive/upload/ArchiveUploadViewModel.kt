...
class ArchiveUploadViewModel @Inject constructor(
    ...
    private val archiveUploadVoListStateFlow: MutableStateFlow<List<ArchiveUploadVo>> =
        MutableStateFlow(emptyList())

    override val uiState: ArchiveUploadPageState = ArchiveUploadPageState(
        title = titleStateFlow.asStateFlow(),
        typeTitle = typeTitleStateFlow.asStateFlow(),
        enableButton = enableButtonStateFlow.asStateFlow(),
        imageCount = imageCountStateFlow.asStateFlow(),
        pageType = pageTypeStateFlow.asStateFlow(),
        archiveUploadVoList = archiveUploadVoListStateFlow.asStateFlow(),
    )
    ...
    private fun updateListState(stateList: List<ArchiveUploadVo>) {
        val imageCount = stateList.size - NOT_IMAGE_SIZE
    ...
    private fun getMergedList(title: String): List<ArchiveUploadVo> {
        return listOf(
            ArchiveUploadVo(viewType = ArchiveItemViewType.EDIT_VIEW, editText = title),
            ArchiveUploadVo(viewType = ArchiveItemViewType.IMAGE_TEXT_VIEW)
        )
    }
    ...
    private fun addList(vo: ArchiveUploadVo) {
        ...
        originList.add(vo)
        val imageCount = originList.size - NOT_IMAGE_SIZE
        val last = arrayListOf(ArchiveUploadVo(viewType = ArchiveItemViewType.IMAGE_ADD_VIEW))
        if (imageCount < MAX_IMAGE) updateListState(originList + last) else updateListState(
            originList
        )
    }
    ...
    private fun editArchive() {
        val mergeList = getImageList()
        val request = EditArchiveRequestVo(
            plubbingId,
            archiveId,
            ArchiveContentRequestVo(editText, mergeList)
        )
        viewModelScope.launch {
