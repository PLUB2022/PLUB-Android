...
class PutScheduleAttendUseCase @Inject constructor(
    private val scheduleRepository: ScheduleRepository
) : UseCase<PutScheduleAttendRequestVo, Flow<UiState<CalendarAttendVo>>>() {
    override suspend operator fun invoke(request: PutScheduleAttendRequestVo): Flow<UiState<CalendarAttendVo>> {
        return scheduleRepository.putScheduleAttend(request)
    }
}